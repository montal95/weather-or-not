...
const runSearch = () => {
    ...
    var APIKey2 = "F54541AQV51HLSQCB0FLQLOL5QQQSEICCZZDCVRVVFXJKZSM";
    ...
    var queryURL = `https://api.openweathermap.org/data/2.5/forecast?q=${userInput},USA${APIKey1}&count=5`;
   ...
   //openWeather call
    $.ajax({
      ...
      console.log("shit"+response.list[0].weather[0].description);
    ...
    $.ajax({
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        let $row = $("<div>").addClass("row venue").css("border-radius", "25px");
        ...
        $.ajax({
          ...
          for (let t = 0; t < photo.length; t++) {
            ...
            contentString += `<img class="rounded" src="${photo[t].prefix}200x200${photo[t].suffix}">`

            
            //This is the html for the pic that will be used in the normal html container...pretains to homepage list
            let $pic = $("<div>").html(`<img class="rounded" src="${photo[t].prefix}200x200${photo[t].suffix}">`);
...
$("#topPicks").on("click", "#mapView" , function(){
  $("#map").fadeIn("3000");
  $("#jumbo").fadeOut("3000");
  $("#weatherFore").fadeOut("3000");
  $("#close").fadeIn("3000");
  $("#topPicks").fadeOut("3000");
  $.backstretch("");
...
$("#close").on("click", function(){
  $("#map").fadeOut("3000");
  $("#weatherFore").fadeIn("3000");
  $("#jumbo").css("display","block");
  $("#topPicks").fadeIn("3000");
  $("#close").fadeOut("3000");
...
$("#topPicks").on("click", ".rounded" , function(){
